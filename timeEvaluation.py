"""
This file shows the difference in time when converting dataframe to other formats.
Check file path fron line 15 to line 23 before executing the file.
"""
...
def cleanData(yearSeason, form):  
    ...
    try:
        ...
        df = pd.read_csv(readLoc+yearSeason+'_A_lvr_land_'+form+'.CSV', index_col=False, encoding = 'ansi', skiprows=range(1, 2))
    ...
    # Find columns that need to change (string to int)
    needToChange=[]
    ...
    dic_Q1=df.groupby(['鄉鎮市區', '建物型態'])['單價(元/平方公尺)'].quantile(.25).to_dict()
    dic_Q3=df.groupby(['鄉鎮市區', '建物型態'])['單價(元/平方公尺)'].quantile(.75).to_dict()    
       
    groupIQRrange=collections.defaultdict(list) # contain good data range of each district
    for key in dic_Q1:
    ...
    for index,row in df.iterrows():
        if not pd.isnull(row['單價(元/平方公尺)']):
            if row['單價(元/平方公尺)']>groupIQRrange[(row['鄉鎮市區'], row['建物型態'])][1][1] or row['單價(元/平方公尺)']<groupIQRrange[(row['鄉鎮市區'], row['建物型態'])][1][0]:
                outlierBook.append((index, 2))
                farOutlierCount+=1
                df.at[index, 'outlier']=2
            elif row['單價(元/平方公尺)']>groupIQRrange[(row['鄉鎮市區'], row['建物型態'])][0][1] or row['單價(元/平方公尺)']<groupIQRrange[(row['鄉鎮市區'], row['建物型態'])][0][0]:
    ...
    print("To CSV costs : %s seconds. " % (time2 - time1))
    print("To msgpack costs : %s seconds. " % (time3 - time2))
    print("To sqlite3 costs : %s seconds. " % (time4 - time3))
